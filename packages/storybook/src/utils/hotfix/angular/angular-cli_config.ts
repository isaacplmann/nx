...
export function applyAngularCliWebpackConfig(
  ...
  if (!isBuildAngularInstalled()) {
    logger.info(
      '=> Using base config because @angular-devkit/build-angular is not installed.'
    );
    return baseConfig;
  }
  ...
  const entry = [
    ...baseConfig.entry,
    ...Object.values(cliStyleConfig.entry).reduce(
      (acc: any, item) => acc.concat(item),
      []
    )
  ];
  ...
  const resolve = {
    ...
    modules: Array.from(
      new Set([
        ...
        ...cliCommonConfig.resolve.modules
      ])
    ),
    plugins: [
